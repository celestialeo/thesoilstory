  ...
  return (
    <>
      <div className="w-full h-full relative">
        <Header text="white" />
        <div className="flex flex-col md:flex-row justify-between px-4 md:px-40 items-center gap-10">
          <button
          ...
          <div className="w-full mt-4 md:mt-0 text-center md:text-left">
            <h3 style={{ fontFamily: "Caveat" }} className="text-[#4BAF47] text-4xl mb-5">
              Our Team
            </h3>
            <h1 className="text-3xl md:text-6xl font-semibold mb-2">
              {TeamData[current]?.name}
            </h1>
            <h6 className="text-sm text-[#714015]">{TeamData[current]?.tag}</h6>
            <p className="w-full md:w-[80%] mt-6">{TeamData[current]?.about}</p>
          </div>
          <div className="w-40 md:w-auto">
            <img src={TeamData[current]?.image} className="w-full h-auto" alt={TeamData[current]?.name} />
          </div>
          <button
            className="hover:bg-gray-400 text-gray-800 font-bold rounded-full p-1 border"
            onClick={handleNext}
        ...
        <div className="flex justify-start z-10 py-5 glassEffect2 absolute bottom-0 w-full rounded-none rounded-t-lg px-4 md:px-36 gap-4 md:gap-14">
          {TeamData.map((data, idx) => (
            <div className="text-center hover:scale-[1.1]" key={idx}>
              <img
                src={data.avatar}
                alt={data.shortName}
                className="mx-auto rounded-full w-16 md:w-[77px]"
              />
